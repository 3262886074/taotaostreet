...
<div class="am-container header">
    ...
    <ul class="message-r">
        ...
        <div class="topMessage mini-cart">
            <div class="menu-hd">
                <a id="mc-menu-hd" href="${ctx}/shopping/shoppingCart" target="_top"><i
                        class="am-icon-shopping-cart  am-icon-fw"></i><span>购物车</span></a>
            </div>
        </div>
        <div class="topMessage favorite">
            <div class="menu-hd">
                <a href="${ctx}/users/allCollects" target="_top"><i class="am-icon-heart am-icon-fw"></i><span>收藏夹</span></a>
            </div>
            </c:if>
            <c:if test="${empty users}">
            <div class="topMessage my-shangcheng">
        ...
        <div class="tb-detail-list">
            ...
            <ul class="tm-ind-panel">
                <li class="tm-ind-item tm-ind-sellCount canClick">
                    <div class="tm-indcon">
                        <span class="tm-label">月销量</span><span
                            class="tm-count">${commodity.commoditySell.monthSell }</span>
                    </div>
                </li>
                <li class="tm-ind-item tm-ind-sumCount canClick">
                    <div class="tm-indcon">
                        <span class="tm-label">累计销量</span><span
                            class="tm-count">${commodity.commoditySell.allSell }</span>
                    </div>
                </li>
                <li class="tm-ind-item tm-ind-reviewCount canClick tm-line3">
                    <div class="tm-indcon">
                        <span class="tm-label">累计评价</span><span
                            class="tm-count">${commodity.commoditySell.allComm }</span>
                    </div>
                </li>
            </ul>
            <div class="clear"></div>
                                        ...
                                        <ul>
                                            <c:forEach items="${commodity.commodityTypes }" var="l">
                                                <li class="sku-line ">${l.typeName }<i></i> <input
                                                        ...
                                                        id="ct_id" type="hidden" value="${l.ctid }"/>
                                                </li>
                                            </c:forEach>
...
<div class="shopPromotion gold">
    ...
    <div class="coupon">
        ...
        <div class="gold-list">
            <c:if test="${!empty users }">
                <ul>
                    <a href="javascript:void(0);" onclick="getyhq();">
                        <li><i id="q1">125</i>减<i id="q2">5</i></li>
                    </a>
                    <a href="javascript:void(0);" onclick="getyhq1();">
                        <li><i id="a1">198</i>减<i id="a2">10</i></li>
                    </a>
                    <a href="javascript:void(0);" onclick="getyhq2();">
                        <li><i id="w1">298</i>减<i id="w2">30</i></li>
                    </a>
                </ul>
            </c:if>
            <c:if test="${empty users }">
...
<div class="pay">
    ...
    <form id="buyForm" action="" method="post">
        <input type="hidden" id="cid" name="cid" value="${commodity.cid }">
        <input type="hidden" id="ctidsel" name="ctid">
        <input type="hidden" id="ccidsel" name="ccid">
    ...
    <script type="text/javascript">
        function show() {
            if (mon + m2 <= 0) {
                ...
                //在这里提交下面这个表单 修改form的action为一键购买的action
                var form = $("#buyForm");
                form.attr("action", "${ctx}/shopping/buyNow");
                form.submit();
                <%--window.location.href = "${ctx}/shop/standpoint/"--%>
                    <%--+ $("#cid").val() + "/" + $("#ctidsel").val() + "/"--%>
                    <%--+ $("#ccidsel").val() + "/" + $("#ttNumber").val() +--%>
                    <%--"/" + $("#onemoney").val();--%>
            } else {
                $("#show1").show();
            }
    ...
    <script type="text/javascript">
        function show2() {
            if (mon + m2 <= 0) {
                ...
                $("#ttNumber").attr("value", tt);
                $("#onemoney").attr("value", oneMoney);
                //在这里提交下面这个表单 修改form的action为加入购物车的action
                <%--var form = $("#buyForm");--%>
                <%--form.attr("action", "${ctx}/shopping/addShoppingCart");--%>
                var url = "${ctx}/shopping/addShoppingCart";
                var cid = $("#cid").val();
                var ctid = $("#ctidsel").val();
                var ccid = $("#ccidsel").val();
                var number = $("#ttNumber").val();
                console.info("数量:",number)
                var money = $("#onemoney").val();
                var args = {cid:cid,ctid:ctid,ccid:ccid,number:number,money:money};
                $.post(url,args,function (data) {
                    if (data == 1) {
                        alert("添加商品成功");
                    }
                })
                <%--window.location.href = "${ctx}/shop/addShopping_Cart/"--%>
                            <%--+ $("#cid").val() + "/" + $("#ctidsel").val() + "/"--%>
                        <%--+ $("#ccidsel").val() + "/" + $("#ttNumber").val() +--%>
                        <%--"/" + $("#onemoney").val();--%>
            } else {
                        ...
                        <ul class=" tb-taglist am-avg-sm-4">
                            <li class="tb-taglist-li tb-taglist-li-current">
                                <div class="comment-info">
                                    <span>全部评价</span> <span
                                        class="tb-tbcr-num">(${commodity.commoditySell.allComm })</span>
                                </div>
                            </li>
                            ...
                            <li class="tb-taglist-li tb-taglist-li-1">
                                <div class="comment-info">
                                    <span>好评</span> <span
                                        class="tb-tbcr-num">(${commodity.commoditySell.goodcomm })</span>
                                </div>
                            </li>
                            ...
                            <li class="tb-taglist-li tb-taglist-li-0">
                                <div class="comment-info">
                                    <span>中评</span> <span
                                        class="tb-tbcr-num">(${commodity.commoditySell.midcomm })</span>
                                </div>
                            </li>
                        ...
                        <c:forEach items="${commodity.comments }" var="cc">
                            <li class="am-comment">
                                <!-- 评论容器 --> <a href=""> <img class="am-comment-avatar"
                                                               src="${ctx}/resources/images/hwbn40x40.jpg"/>
                                <!-- 评论者头像 -->
                            </a>
